# NAMI morning briefing task

Prepare today's NAMI Post Ideas bulletin.

Read the public Network directory, the enabled source registry, recent idea history, member feature history and configuration. Research current North East creative news and upcoming events. Produce the structured briefing defined by the attached `nami-post-ideas` skill.

Score candidate ideas using:

- North East relevance: 30
- Fit with NAMI and the Creative Network: 25
- Freshness or originality: 20
- Timeliness: 15
- Ease of producing today: 10

Only the strongest material should reach the email. A shorter verified briefing is better than filling a section with weak ideas.

After successful delivery, append the run to `Run Log`, append every suggestion to `Idea Archive`, and update `Last suggested`, `Last angle`, `Times suggested` and `Cooldown until` for selected members.

While `Config!test_mode` is `TRUE`, deliver only to the configured test recipient. Do not update `Last published` or `Times published` until Joe marks an idea as used.

