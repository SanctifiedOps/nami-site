...
async function sendOwnerAlert(job: typeof schema.ownerAlertJobs.$inferSelect) {
  ...
  const [application] = job.kind === "application"
    ...
    : [];
  const [event] = job.kind === "event"
    ? await db.select().from(schema.networkEvents).where(eq(schema.networkEvents.id, job.recordId)).limit(1)
    : [];
  ...
  const applicationDetails = application ? `<table role="presentation" style="width:100%;border-collapse:collapse;margin:20px 0 26px"><tbody>${row("Name", application.displayName)}${row("Email", application.email, `mailto:${application.email}`)}${row("Instagram", application.instagramUrl ?? "")}${row("Category", application.requestedCategory)}${row("Location", application.location)}${row("Project link", application.websiteUrl ?? "", application.websiteUrl ?? undefined)}${row("What NAMI should know", application.bio)}${row("Source", "namicreative.co.uk/network")}${row("Submitted", application.submittedAt.toISOString())}</tbody></table>` : "";
  const eventDetails = event ? `<table role="presentation" style="width:100%;border-collapse:collapse;margin:20px 0 26px"><tbody>${row("Event", event.title)}${row("Type", event.eventType)}${row("Starts", event.startsAt.toISOString())}${row("Ends", event.endsAt?.toISOString() ?? "")}${row("Venue", event.venue)}${row("Location", [event.address,event.location,event.region].filter(Boolean).join(", "))}${row("Format", event.format.replaceAll("_", " "))}${row("Price", [event.priceType,event.priceDetails].filter(Boolean).join(": "))}${row("Summary", event.summary)}${row("Information link", event.bookingUrl ?? "", event.bookingUrl ?? undefined)}${row("Submitted", event.submittedAt.toISOString())}</tbody></table>` : "";
  const details = applicationDetails || eventDetails || `<p style="font-size:17px;line-height:1.6;color:#d4d4d8">${encode(job.body)}</p>`;
  const heading = application ? "Creative Network submission" : event ? "Creative Network event submission" : encode(job.heading);
  const actionLabel = application ? "Review and approve application" : event ? "Review this event" : "Open in the owner dashboard";
  const html = `<!doctype html><html><body style="margin:0;background:#242426;color:#f7f7f7;font-family:Arial,sans-serif"><div style="max-width:700px;margin:auto;padding:42px 24px"><p style="color:#ff00a8;font-weight:800">NAMI <span style="color:#fff;font-weight:400">CREATIVE</span></p><h1 style="font-size:28px;line-height:1.1">${heading}</h1>${details}<p style="margin:28px 0"><a href="${encode(job.actionUrl)}" style="display:inline-block;background:#ff00a8;color:#fff;text-decoration:none;padding:14px 22px;border-radius:999px;font-weight:700">${actionLabel}</a></p>${application ? `<p style="color:#b7b7bd;font-size:13px">Reply to this email to contact the submitter.</p>` : `<p style="color:#9ca3af;font-size:13px">Reference: ${encode(job.recordId)}</p>`}</div></body></html>`;
